...
def insert_default_printformat_elements(db):
    ...
    r = [
        ...
        { 'name': 'default', 'sorting_key': 'acquisition_date', 'table_name': 'DicomSeries',
          'format': '{id:4} [{acquisition_id}] {dicom_study.patient.name: <10} {injection.radionuclide.name} {time_from_inj} {injection.cycle} {injection.activity_in_mbq} {modality} {content_type} {acquisition_date:%Y-%m-%d-%H:%M} {image_size}x{nb_dicom_files} {image_spacing} / {series_description} / {dicom_study.study_description} / {dicom_study.study_name} / {dataset_name} / {labels}'
        },
        ...
        {'name': 'default', 'sorting_key': 'date', 'table_name': 'Acquisition',
         'format': '{id:4} [{injection_id}] {date:%Y-%m-%d-%H:%M} {modality} {fov} {labels}'
         },

        {'name': 'default', 'table_name': 'Listmode',
         'format': '{id:4} [{acquisition_id}] {date} {modality} {content_type} {file_id} {labels}'
         },
